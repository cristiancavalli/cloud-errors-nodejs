 ...
 * @param {commonDiag.logger} logger - an inited logger instance
 */
var Configuration = function(givenConfig, logger) {
  ...
  /**
   * The _startedInit property denotes whether or not the Configuration instance
   * has started the init process.
   * @memberof Configuration
   * @private
   * @type {Boolean}
   * @defaultvalue false
   */
  this._startedInit = false;
  /**
  ...
  /**
   * The _logger property holds a logger instance which, if valid, will be used
   * to log any debug or error information generated by the instance.
   * @memberof Configuration
   * @private
   * @type {Object|Null}
   * @defaultvalue null
   */
  this._logger = isObject(logger) ? logger : null;
}
...
Configuration.prototype._checkLocalProjectId = function() {
  if (isString(env.GCLOUD_PROJECT) && !isEmpty(env.GCLOUD_PROJECT)
...
Configuration.prototype._checkLocalProjectNumber = function() {
  if (isString(this._projectNumber)) {
...
/**
 * Returns the _initError property on the instance.
 * @memberof Configuration
 * @public
 * @function getError
 * @returns {Error|Null} - returns the _initError property
 */
Configuration.prototype.getError = function() {
  return this._initError;
};
/**
 * Returns a Boolean based upon whethe the Configuration instance communicated
 * with the metadata service and has a sufficient configuration.
 * @memberof Configuration
 * @public
 * @function isReady
 * @returns {Boolean} - returns true if instance initialized without error,
 *  otherwise false
 */
Configuration.prototype.isReady = function() {
  return this._initState && !this._initError;
};
/**
 * Returns a Boolean indicating whether or not the Configuration instance went
 * through the initialization process and encountered an error during that
 * process.
 * @memberof Configuration
 * @public
 * @function hasErrored
 * @returns {Boolean} - returns true if _initError is set as an Error, otherwise
 *  false
 */
Configuration.prototype.hasErrored = function() {
  return this._initError instanceof Error;
};
/**
 * The init function causes the Configuration instance to being the
 * initialization process of gathering configuration from both the metadata
 * service, runtime configuration and environment. This function will begin the
 * init process only once. The function will defer starting the initialization
 * process on any successive calls.
 * @memberof Configuration
 * @public
 * @function init
 * @chainable
