...
RequestHandler.prototype.sendError = function(errorMessage, userCb) {
  ...
  function doRequest() {
    var projectIdentifier = (that._config.getProjectId()) ? 
      that._config.getProjectId() : that._config.getProjectNumber();
    that._request({
      url: getErrorReportURL(projectIdentifier, that._config.getKey()),
      method: 'POST',
      json: errorMessage
    }, cb);
  }

  if (that._config.isReady()) {
    if (that._config.getShouldReportErrorsToAPI()) {
      doRequest();
    } else {
      cb(new Error([
  ...
  } else if (that._config.hasErrored()) {
    cb(that._config.getError(), null, null);
  } else {
    // still initing
    that._config.addReadyListener(
      that.sendError.bind(that, errorMessage, userCb));
    that._config.addErrorListener(function (e) {cb(e, null, null);});
  }
};
