...
function hapiErrorHandler(req, err, config) {
  ...
  if (isObject(config) && isFunction(config.getServiceContext)) {
...
function handlerSetup(client, config) {
  var hapiPlugin = {register : hapiRegisterFunction.bind(null, client, config)};
  var version = isObject(config) && isFunction(config.getVersion) ?
