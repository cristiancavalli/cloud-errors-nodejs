...
test(
  ...
  function (t) {
    ...
    client = new RequestHandler(new Configuration({key: key}).init());
    var er = new Error("_@google_STACKDRIVER_INTEGRATION_TEST_ERROR__");
    var em = new ErrorMessage()
      .setMessage(er.stack);
    var s = nock(
...
test(
  ...
  function (t) {
    ...
    client = new RequestHandler(new Configuration().init());
...
test(
  ...
  function (t) {
    ...
    var em = new ErrorMessage()
      .setMessage(er.stack);
    var cfg = new Configuration().init().addErrorListener(function(){});
    client = new RequestHandler(cfg);
...
test(
  ...
  test(
    'Test given invlaid init parameters we should be able to create an errored client',
    function ( t ) {
      var er = new Error("_@google_STACKDRIVER_INTEGRATION_TEST_API_KEY_ERROR__");
      var em = new ErrorMessage()
        .setMessage(er.stack);
      ...
      delete process.env.STUBBED_API_KEY;
      var cfg  = new Configuration().init().addErrorListener(function () {
        t.pass('Configuration should error');
        client.sendError(em, function (er) {
          t.assert(
            er instanceof Error,
            "The function should callback with an error"
          );
          t.pass("Should callback to given function");
          process.env.GCLOUD_PROJECT = oldProject;
          process.env.STUBBED_API_KEY = oldKey;
          t.end();
        })
      }).addReadyListener(function () {
        t.fail('Configuration should error');
        process.env.GCLOUD_PROJECT = oldProject;
        process.env.STUBBED_API_KEY = oldKey;
        t.end();
      });
